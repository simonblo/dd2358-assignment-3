...
TYPE_U32    = ctypes.c_uint
TYPE_F64_PP = ctypes.POINTER(ctypes.POINTER(ctypes.c_double))

clib = ctypes.CDLL("./task_2_9.so")
clib.jacobi_iteration_c.argtypes = [TYPE_U32, TYPE_F64_PP, TYPE_F64_PP]
clib.jacobi_iteration_c.restype  = None
...
def jacobi_iteration_c(f, n):
    g = numpy.zeros((n, n))
    clib.jacobi_iteration_c(TYPE_U32(n), f.ctypes.data_as(TYPE_F64_PP), g.ctypes.data_as(TYPE_F64_PP))
    return g
...
if __name__ == "__main__":
    ...
    f.append((init_grid_c,      jacobi_iteration_c,      [], [], "C"))
    for n in xticks:
        k = len(f)
        for i in range(k):
            g = f[i][0](n)
